...
(function (window, document, $) {
	...
	$.extend(F, {
		...
		isImage: function (str) {  //  "||"后面的条件，是 依照项目需要，url种包含.action默认返回图片流 
			return (str && str.match(/\.(jpg|gif|png|bmp|jpeg)(.*)?$/i)) || (str && str.match(/\.(action)(.*)?$/i));
		},
		...
		_preloadImages: function() {
			...
			for (var i = 1; i <= Math.min(current.preload, len - 1); i++) {
				...
				href = item.href || item;  //$( item ).attr('href')
